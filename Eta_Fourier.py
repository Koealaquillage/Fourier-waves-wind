...
# -*- coding: utf-8 -*-
"""
Created on Thu Apr 16 13:08:33 2020
Here we measure the spectrum for overlevatin
...
#    time = dates.num2date(time)
    eta = data_struc['OLZO2016_Cris'][0][0][stat][w][0][0][0,:]
    ...
    w = np.blackman(len_eta)
    fft_eta = np.fft.rfft(eta*w)
    # Now we take the product to be real
    fft_eta_2 = fft_eta*fft_eta.conj()
    # And we smooth it
    fft_eta_2 = gaussian_filter1d(fft_eta_2.real,sigma =3)
    # And for the frequency
    freq = np.fft.rfftfreq(len_eta, d= t_samp)
    # And we plot
    ax_wave.semilogy(freq, fft_eta_2, label=stat)

ax_wave.legend(loc='best')
ax_wave.set_ylabel('Overelevation power (m^2)')
